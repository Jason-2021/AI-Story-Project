"""
image_generator/run_test.py — 獨立測試腳本：圖片生成

用法（從專案根目錄執行）：
  # 單集
  python image_generator/run_test.py --input-dir workspace/test_001/ep01

  # 所有集數
  python image_generator/run_test.py --input-dir workspace/test_001

  # 指定集數
  python image_generator/run_test.py --input-dir workspace/test_001 --episodes 1-3
  python image_generator/run_test.py --input-dir workspace/test_001 --episodes 1,3
"""
import sys
...
def _parse_args():
    p = argparse.ArgumentParser(description="Image Generator Standalone Test")
    ...
    p.add_argument("--provider",  type=str, default="gemini", help="圖片生成供應商")
    return p.parse_args()
...
async def _process_ep(ep_dir: Path, provider: str):
    ...
    results = await generate_images_router(all_scenes, output_dir=image_dir, provider=provider)
    (image_dir / "image_results.json").write_text(
        json.dumps([r.model_dump() for r in results], ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    print(f"  ✅ {ep_dir.name}: {len(results)} 張圖片 → {image_dir}")
...
async def main():
    ...
    if (input_dir / "script.json").exists():
        # 單集模式
        print(f"\n🖼️  [ImageTest] 單集模式：{input_dir.name}")
        ...
        if not ep_dirs:
            ...
            return
        print(f"\n🖼️  [ImageTest] 系列模式：處理 {len(ep_dirs)} 集")
