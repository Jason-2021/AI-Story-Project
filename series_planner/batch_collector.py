...
collect 完成後（image 存 PNG、audio 存 WAV + Whisper）：
  → 標記 images/audio 為 "completed"
  → 直接 handoff 給現有 realtime pipeline（run_episode / run_series_mode / run_anthology_mode）
  Stage 3（video render）、BGM、merge 全由 realtime pipeline 負責，行為與非 batch 完全一致。
"""
...
from audio_generator.audio_router import _run_whisper, AudioResult
...
async def _fallback_audio(meta: dict, sample_rate: int) -> AudioResult:
    ...
    return AudioResult(scene_id=scene_id, file_path=str(wav_path),
                       duration=duration, timestamps=timestamps)
...
async def run_batch_collect(batch_id: str) -> None:
    ...
    # ── 4. Write results + mark stages completed ─────────────────────
    unique_run_ids = list(dict.fromkeys(m["run_id"] for m in image_info["scenes"]))
    ...
    batch_path.write_text(json.dumps(batch_data, ensure_ascii=False, indent=2), encoding="utf-8")
    print("\n✅ [BatchCollect] Image + Audio 收取完成，移交 realtime pipeline...")

    # ── 6. Handoff to realtime pipeline ─────────────────────────────
    if mode == "solo":
        from series_planner.episode_runner import run_episode
        args = batch_data["solo_args"]
        await run_episode(
            run_id=sid,
            topic=args["topic"],
        ...
        from series_planner.series_runner import run_series_mode
        await run_series_mode(
            job=batch_data["job"],
            resume_series_id=sid,
            provider_override=batch_data.get("provider_override"),
        )

    elif mode == "anthology":
        from series_planner.series_runner import run_anthology_mode
        await run_anthology_mode(
            job=batch_data["job"],
            resume_anthology_id=sid,
            provider_override=batch_data.get("provider_override"),
        )
